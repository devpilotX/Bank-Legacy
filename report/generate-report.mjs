...
const SECTIONS = [
  ...
  },
  {
    ...
    blocks: [
      'Verification is the heart of what we sell, so I want to explain what it is meant to do, and then tell you exactly how much of it works right now. I tested this part carefully.',
      { h2: 'What it is meant to do' },
      'The promise is simple to say. When we rewrite a piece of COBOL into Java, we want to prove the new Java behaves exactly like the old code. The way you do that is with test cases. A test case is an input and the output the old system gives for that input. You run the new Java on the same input and check you get the same output. Do that for enough cases and you can trust the new piece.',
      { h2: 'What actually works today' },
      'The parts that manage all of this are built and they work. You can add test cases by hand. The AI can draft cases for you to confirm. The tool stores the cases, runs a check, records a pass or fail, and gives a clear reason when something does not match. I ran all of that and it works.',
      { h2: 'The honest gap' },
      'Here is the part I have to be straight about. Today the tool does not actually run the new Java. When you do a check, the tool compares an expected output against an output that you hand it. It does not compile the Java, it does not run it, and it does not produce that output itself. I proved this three ways: I tested it live and the result only depended on the text I typed in, the written API notes say the piece that runs the Java is a later step, and I read the code and there is nothing in there that compiles or runs anything.',
      'So right now a person types in what the Java produced, and the tool checks that against what was expected. The comparison is real and it is careful. But the engine that actually runs the new Java, and ideally runs the old code too, to produce those outputs on its own, is not built yet.',
      'Why this matters so much: it would be easy to look at a row of green passed checks and think the system proved the Java is correct. It did not. A human supplied the answer it checked. Until we build the part that runs the code itself, verification is a careful assistant, not the proof we want it to be. This is the single most important thing to finish.',
    ],
  },
  {
  ...
  },
  {
    ...
    blocks: [
      ...
      { li: 'The verification case management and the output comparison (with the big caveat from section 7 that it does not run the Java itself).' },
      { li: 'The backend starts up, connects to the database, applies its database setup steps, and answers a health check.' },
      { li: 'All the automated tests pass. There are 45 of them across the three parts, and every one passed.' },
  ...
  },
  {
    ...
    blocks: [
      'Now the honest other side. Some things look further along than they are, so here is where the gaps are.',
      { li: 'Verification does not run the Java. This is the big one. As I explained in section 7, today a person supplies the output and the tool only compares it. The engine that compiles and runs the new code is not built. This is the most important missing piece, because it is the core promise.' },
      { li: 'The AI Java translation is a first draft, not finished code. On my test it got the shape right but left notes saying where it still needed work, and it wrapped the code in some extra formatting. It is a useful head start, but an engineer has to finish it. Part of that is because we are running a cheap AI model right now.' },
      { li: 'The cheap AI model. We are on a low cost model at the moment. It is surprisingly good at explaining code, but its Java drafts need real human cleanup. A stronger model would help the translation.' },
  ...
  },
  {
    ...
    blocks: [
      'Here is the to do list, most important first, in plain order.',
      { li: 'Build the engine that actually runs the new Java, and ideally the old code too, and compares them on its own. This is what turns verification from a careful assistant into real proof. Without it, we cannot honestly tell a bank we proved the match. Everything else is less important than this.' },
      { li: 'Improve the translation. A better AI model, and some cleanup of the drafts it returns, so the Java that comes out needs less hand fixing.' },
      ...
      { li: 'Actually build and run the whole thing in containers. The setup for that is written but it has not been built or run on a machine that has the container tools, so it is unproven.' },
      ...
      'None of this is a disaster. The foundation is solid. But the first item, real verification, is the one that has to happen before we can sell the core promise with a straight face.',
    ],
  },
  {
